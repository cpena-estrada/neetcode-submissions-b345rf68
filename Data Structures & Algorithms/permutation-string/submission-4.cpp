...
public:
    bool checkInclusion(string s1, string s2) {
        ...
        unordered_map<char, int> s1_freq; // char, frequency
        unordered_map<char, int> window_freq; // char, frequency | updated live
        int window_len = s1.size();

        // populate freqs for s1
        for (const auto& c : s1) { s1_freq[c]++; }

        // populate freq for s2 with the frist s1.size() characters of s2
        for (int i = 0; i < window_len; i++) { window_freq[s2[i]]++; }
    
        if (s1_freq == window_freq) { return true; } // check that initial window frequency

        int left = 0;
        window_freq[s2[left]]--;
        if (window_freq[s2[left]] == 0) { window_freq.erase(s2[left]); } // make sure you do s2[left] and not just left
        left = 1;
