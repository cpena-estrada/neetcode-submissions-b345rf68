...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        set<pair<int, int>> seen; // node, cost (for optimization purposes)
        ...
        while (!min_heap.empty()) {
            ...
            //unecessary minor optimization. dont exlpore seen path with same cost and same num of stops
            if (seen.count({node1, cost1})) { continue; }
            seen.insert({node1, cost1});
